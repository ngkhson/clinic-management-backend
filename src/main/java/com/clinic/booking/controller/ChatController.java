...
public class ChatController {
    ...
    public void sendPrivateMessage(@Payload ChatMessageRequest chatMessageRequest) {
        ...
        chatMessageRequest.setTimestamp(savedMessage.getTimestamp());
        ...
        if ("STAFF".equals(chatMessageRequest.getReceiverEmail())) {
            ...
            messagingTemplate.convertAndSend("/topic/staff", chatMessageRequest);
            // -> Gửi lại một bản copy cho bệnh nhân để hiển thị trên màn hình của họ
            messagingTemplate.convertAndSend("/queue/messages/" + chatMessageRequest.getSenderEmail(), chatMessageRequest);
        } else {
            // Trường hợp 2: Nhân viên (Admin/Doctor) trả lời bệnh nhân
            // -> Gửi trực tiếp đến kênh riêng của bệnh nhân đó
            messagingTemplate.convertAndSend("/queue/messages/" + chatMessageRequest.getReceiverEmail(), chatMessageRequest);
